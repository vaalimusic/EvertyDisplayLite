...
pub fn translate<'a>(russian: impl Into<Cow<'a, str>>) -> Cow<'a, str> {
    ...
    let english = match russian.as_ref() {
        ...
        "Использовать разрешение и доступную частоту MAIN для виртуальных дисплеев" => "Use MAIN resolution and an available matching refresh rate for virtual displays",
...
fn arabic_translation(russian: &str) -> Option<&'static str> {
    Some(match russian {
        ...
        "Использовать разрешение и доступную частоту MAIN для виртуальных дисплеев" => "استخدام دقة MAIN وأفضل معدل تحديث متاح للشاشات الافتراضية",
...
fn spanish_translation(russian: &str) -> Option<&'static str> {
    Some(match russian {
        ...
        "Использовать разрешение и доступную частоту MAIN для виртуальных дисплеев" => "Usar la resolución de MAIN y la mejor frecuencia disponible en pantallas virtuales",
